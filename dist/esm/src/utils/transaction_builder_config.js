...
export function getTransactionBuilderConfig(protocolParameters, slotConfig, blockfrostConfig) {
    const bucket = [];
    let builderA = C.TransactionBuilderConfigBuilder.new();
    const coinsPerUtxoByte = C.BigNum.from_str(protocolParameters.coinsPerUtxoByte.toString());
    ...
    builderB.free();
    const keyDeposit = C.BigNum.from_str(protocolParameters.keyDeposit.toString());
    bucket.push(keyDeposit);
    builderB = builderA.key_deposit(keyDeposit);
    builderA.free();
    const poolDeposit = C.BigNum.from_str(protocolParameters.poolDeposit.toString());
    ...
    builderB.free();
    builderB = builderA.collateral_percentage(protocolParameters.collateralPercentage);
    builderA.free();
    builderA = builderB.max_collateral_inputs(protocolParameters.maxCollateralInputs);
    builderB.free();
    const maxTxExMem = C.BigNum.from_str(protocolParameters.maxTxExMem.toString());
    bucket.push(maxTxExMem);
    const maxTxExSteps = C.BigNum.from_str(protocolParameters.maxTxExSteps.toString());
    bucket.push(maxTxExSteps);
    const exUnits = C.ExUnits.new(maxTxExMem, maxTxExSteps);
    bucket.push(exUnits);
    builderB = builderA.max_tx_ex_units(exUnits);
    builderA.free();
    const exUnitPrices = C.ExUnitPrices.from_float(protocolParameters.priceMem, protocolParameters.priceStep);
    ...
    builderA.free();
    const blockfrost = C.Blockfrost.new(blockfrostConfig?.url ?? "" + "utils/tx/evaulate", blockfrostConfig?.projectId ?? "");
